...
# SCRIPT_DIR=/g/data/$PROJECT/$USER/TNPgeneration/MDsim  # For Gadi q27
SCRIPT_DIR=$HOME/TNPgeneration/MDsim  # For Gadi hm62
EXAM_LOCK=examine.lock; JOB_LIST=jobList; QUEUE_LIST=queueList; SCRIPT=runAnneal.sh

maxQueueNum=15; numInQueue=$(qselect -u $USER | wc -l); numToSub=$(echo "$maxQueueNum-$numInQueue" | bc)  # qselect: select from queue is for your name; wc := word count per line; -u := username.
