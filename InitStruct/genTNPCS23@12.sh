#!/bin/bash
# Script to generate 23@12 core-shell trimetallic nanoparticles (23@12 CS TNPs).
# Feed different parameters into the LAMMPS input template file genTNPCS23@12.in
            ...
            for ((n=0;n<${#RATIO_LIST[@]};n++)); do
                for ((o=0;o<RANDOM_DISTRIB_NO;o++)); do
                    ...
                    if test -f $outFile; then
                        echo "  $element1$element2$size1$element2$element3$size3$shape${RATIO_LIST[$l]}$((100-${RATIO_LIST[$l]}))${RATIO_LIST[$n]}$((100-${RATIO_LIST[$n]}))CRSR$m$o.lmp already exists, skipping...";
                        continue
                    else
