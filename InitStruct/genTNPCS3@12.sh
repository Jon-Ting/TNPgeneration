#!/bin/bash
# Script to generate 3@12 core-shell trimetallic nanoparticles (3@12 CS TNPs).
# Feed different parameters into the LAMMPS input template file genTNPCS3@12.in
...
cs() {
    ...
    if test -f $outFile; then
        echo "  $element1$size1$shape$element2$size2$shape$element3$size3${shape}CS.lmp already exists, skipping...";
        return
    else
...
csl10() {
    ...
    # Check if the TNP has been generated
    if test -f $outFile; then
        echo "  $element1$element2$size2$element3$size3${shape}CSL10.lmp already exists, skipping...";
        return
    else
...
csral() {
    ...
    for ((l=0;l<${#RATIO_LIST[@]};l++)); do
        for ((m=0;m<RANDOM_DISTRIB_NO;m++)); do
            ...
            # Check if the TNP has been generated
            if test -f $outFile; then
                echo "  $element1$element2$size2$element3$size3$shape${RATIO_LIST[$l]}$((100-${RATIO_LIST[$l]}))CSRAL$m.lmp already exists, skipping...";
                continue
            else
    ...
    for ((j=0;j<3;j++)); do
        ...
        for ((k=0;k<3;k++)); do
            ...
            cs      "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    ...
                    "${FCC_LC_ARR[$k]}" 30 25 15
            cs      "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
                    "${FCC_LC_ARR[$k]}" 30 25 10
            cs      "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
                    "${FCC_LC_ARR[$k]}" 30 20 15
            cs      "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
            ...
            cs      "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
                    "${FCC_LC_ARR[$k]}" 30 15 10
            csl10   "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
                    "${FCC_LC_ARR[$k]}" 30 30 25
            csl10   "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
                    "${FCC_LC_ARR[$k]}" 30 30 20
            csl10   "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
                    "${FCC_LC_ARR[$k]}" 30 30 15
            csl10   "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
