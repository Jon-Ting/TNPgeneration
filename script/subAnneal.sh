...
maxQueueNum=30; numInQueue=$(qselect -u $USER | wc -l); numToSub=$(echo "$maxQueueNum-$numInQueue" | bc)  # qselect: select from queue is for your name; wc := word count per line; -u := username.
