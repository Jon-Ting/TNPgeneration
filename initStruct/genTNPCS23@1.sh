...
cl10s() {
    ...
    if test -f $outFile; then
        echo "  $element1$size1$element2$element3$size3${shape}CL10S.lmp already exists, skipping...";
        return
    else
...
crals() {
    ...
    for ((l=0;l<${#RATIO_LIST[@]};l++)); do
        for ((m=0;m<RANDOM_DISTRIB_NO;m++)); do
            ...
            # Check if the TNP has been generated
            if test -f $outFile; then
                echo "  $element1$size1$element2$element3$size3$shape${RATIO_LIST[$l]}$((100-${RATIO_LIST[$l]}))CRALS$m.lmp already exists, skipping...";
                continue
            else
    ...
    for ((j=0;j<3;j++)); do
        ...
        for ((k=0;k<3;k++)); do
            ...
            cl10s   "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
                    "${FCC_LC_ARR[$k]}" 30 15 15
            cl10s   "${ELEMENT_ARR[$i]}" "${MASS_ARR[$i]}" "${RADIUS_ARR[$i]}" \
                    "${ELEMENT_ARR[$j]}" "${MASS_ARR[$j]}" "${RADIUS_ARR[$j]}" \
                    "${ELEMENT_ARR[$k]}" "${MASS_ARR[$k]}" "${RADIUS_ARR[$k]}" \
                    "${FCC_LC_ARR[$k]}" 30 10 10
